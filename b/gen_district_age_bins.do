...
// }
// 
// /* drop a weird tiny duplicate rural district */
// use $tmp/secc_age_bins_r, clear
// drop if pc11_state_id == "12" & pc11_district_id == "246" & secc_pop_r == 198
// save $tmp/secc_age_bins_r, replace
...
/* open urban data */
use $tmp/secc_age_bins_u, clear

/* merge with rural data */
merge 1:1 pc11_state_id pc11_district_id using $tmp/secc_age_bins_r
...
/* urban */
merge 1:1 pc11_state_id pc11_district_id using $iec1/pc11/pc11u_pca_district_clean, keepusing(pc11_pca_tot_p) assert(match using) 
ren pc11_pca_tot_p pc11_pca_tot_u
ren _merge _merge_u

/* rural */
merge 1:1 pc11_state_id pc11_district_id using $iec1/pc11/pc11r_pca_district_clean, keepusing(pc11_pca_tot_p) assert(match)
...
/* assign them the mean state age shares for districts with no data. */
foreach loc in r u {
  foreach age in $agebins {
...
/* recalculate age shares from clean/imputed data */
drop *share
foreach loc in r u {
...
/* impute population in each age bin from SECC age share and total population */
foreach i in $agebins {
...
foreach l in t r u {
  ...
  /* open the data */
  use pc11_pca_tot_`l' pc11_state_id pc11_district_id age_*_`l'_share age_*_`l' using $iec/health/covid_data/secc_age_bins_t, clear
  ...
  /* note: south korea aggregate CFR measured on March 24 was 1.33% */
  ren italy cfr
  ...
  bys pc11_state_id pc11_district_id: egen district_estimated_cfr_`l' = total(est_p_death_age)
  ...
  replace district_estimated_cfr_`l' = district_estimated_cfr_`l' * $india_multiplier
  ...
  reshape wide age_, i(pc11_state_id pc11_district_id district* pc11_pca_tot_`l') j(age_bins)
...
use $tmp/cfr_t
merge 1:1 pc11_state_id pc11_district_id using $tmp/cfr_r, assert(match) nogen
merge 1:1 pc11_state_id pc11_district_id using $tmp/cfr_u, assert(match) nogen

order pc11_state_id pc11_district_id pc11_pca* district_estimated_cfr_*

save $covidpub/district_age_dist_cfr, replace
