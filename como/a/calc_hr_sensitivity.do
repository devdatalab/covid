...
cap erase $tmp/prrs_b.csv
append_to_file using $tmp/prrs_b.csv, s(prr_ratio)
cap erase $tmp/deaths_b.csv
append_to_file using $tmp/deaths_b.csv, s(country,death_share)
...
forval b = 1/1000 {
  ...
  qui foreach prev in india uk_nhs_matched {
    ...
    /* TEMP: get this working with the core HR file */
  ...
  qui {
    ...
    qui sum prr_ratio
    append_to_file using $tmp/prrs_b.csv, s(`r(mean)')
  }
  ...
  qui { 
    ...
    append_to_file using $tmp/deaths_b.csv, s(india,`share')
    sum uk_full_deaths if age < 60
    local share = (`r(N)' * `r(mean)' * 100)
    append_to_file using $tmp/deaths_b.csv, s(england,`share')
  }
}

/* plot PRR bootstrap distribution */
import delimited using $tmp/prrs_b.csv, clear
sum prr_ratio, d
kdensity prr_ratio, xline(`r(mean)')
graphout prr_ratio_bootstrap

/* plot deaths under 60 bootstrap distribution */
import delimited using $tmp/deaths_b.csv, clear
