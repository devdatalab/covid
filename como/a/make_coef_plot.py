...
label_key = {'male_ratio': "Male",
 ...
 'asthma_ocs_ratio': "Severe Asthma",
...
for p in ax.patches:
    ...
    if p.get_width() > 0:
        ax.annotate(note, (p.get_width() + 0.3, p.get_y()+.1), fontsize=14, fontweight="bold", color="#383838")
    else:
        ax.annotate(note, (p.get_width() - 2, p.get_y()+.1), fontsize=14, fontweight="bold", color="#383838")

# format axes
ax.set_xlim([-10,10])
ax.set_ylim([-0.5,17.5])
ax.annotate("India", (4,17), color="Black", fontsize=14)
ax.annotate("UK", (-6,17), color="#e38800", fontsize=14)
ax.set_title("Relative Contribution to Population Mortality", fontsize=16, color="#383838")

# save figure
plt.savefig(os.path.join(homedir, "public_html", "png", "coefplot.png"), bbox_inches="tight", dpi=200)